...
app.config["MAIL_USERNAME"] = "cheirosodobanho@gmail.com"  # Email real
...
def registo():
    if request.method == "POST":
        ...
        senha_segura = generate_password_hash(pw)
        ...
        try:
            with db.cursor() as cursor:
                ...
                cursor.execute(sql, (user, senha_segura, email))
                db.commit()
...
def login():
    if request.method == "POST":
        ...
        with db.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT * FROM utilizadores WHERE username=%s",
                (user,),
            )
            utilizador = cursor.fetchone()
        db.close()

        if utilizador and check_password_hash(utilizador["password"], pw):
